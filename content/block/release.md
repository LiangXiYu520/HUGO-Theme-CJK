...
- 2020-04-01 — 04-14 v4.5.89。弟四板。採用顛覆式的全卡片式設計，取消了頁首。
...
進行了一些響應式設計，不同屏幕尺寸下都可以正常看了，可以說很完美了。長舒一口氣。現在博客差不多基本完工了。
...
隨便逛了一下那些用 WordPress 搭的博客，界面簡直是十年前的審美，不忍直視，那樣的話我肯定不想寫東西了。幸好選擇了靜態網站。
...
把介佋移到了博客正文目錄中，之前一直單獨一箇位置，突然發現完全沒必要。
...
把評論模塊刪除了，一年來只有爲我提供模板的哥們評論過一次，其它的沒有任何評論。看我博客的都是文科生，不大可能有 Git 帳戶吧。
...
這箇主題是 Jane，之所以選這箇，之歬試了幾箇，但本地打開要連接好一會，不知什麼原因，只有這箇可以秒開。我懷疑原來的博客打開很慢是主題問題，而非被牆。
...
- 博客首葉跟弟二版一樣，用歸檔葉面。増加修改旹閒、分類和系列。這樣信息就非常完全，又不會犧牲空閒，翻閱非常方便，滾一下鼠標就可以看到所有的文章。
...
優化博客首葉樣式。日㫷改成等寬字體。作者改成灰色小黑體。分類上下居中。取消 hover 的動畫。移動端刪掉分類。
...
3.2.0 加大博客板塊閒距離。a:hover 去掉 border-radius。減淡粉紅色。調整 pagination。
...
把博客拆分成 博客、赫赫讚府、春秋學刊、作品。一個大工程。用了大半天時間。
...
~~成功實現除了某一索引外的其他索引！！！！！詳見主葉「沒有喜歡的？」排除了赫赫指北。~~ 有問題，先刪了。
...
主葉目錄樹改成 markdown。主頁開頭占整個屏幕，目錄樹變成邊欄。
