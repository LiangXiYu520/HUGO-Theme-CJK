...
我這當然不是準確的「本周」，我算的是向前減三天，向後加四天，因爲每天是從 0 點開始算的。一天 86,400s，一年 315,360s，一閏年 316,224s。我將弟 4、8 年設爲閏年。于是思路很簡單了：

```html
{{ $1date := sub .Date.Unix 31276800  }}
{{ $2date := sub .Date.Unix 31881600 }}
{{ $3date := sub .Date.Unix 62812800 }}
{{ $4date := sub .Date.Unix  63417600 }}
{{ $5date := sub .Date.Unix 94348800 }}
{{ $6date := sub .Date.Unix  94953600 }}
{{ $7date := sub .Date.Unix 125971200 }}
{{ $8date := sub .Date.Unix 126576000 }}
{{ $9date := sub .Date.Unix 157507200 }}
{{ $10date := sub .Date.Unix 158112000 }}
{{ $11date := sub .Date.Unix 189043200 }}
{{ $12date := sub .Date.Unix 189648000 }}
{{ $13date := sub .Date.Unix 220579200 }}
{{ $14date := sub .Date.Unix  221184000 }}
{{ $15date := sub .Date.Unix 252201600 }}
{{ $16date := sub .Date.Unix  252806400 }}
<h2>過去的本周</h2>
<ul>
...
主葉上「分類」的目的是讓新讀者快速瞭解网誌所有文章類型，所以我想在每個分類下面生成兩項最近發布和兩項隨機推薦。同時，分類後面有「赫赫指北」和「推介」兩個系列，需要把它們排除了。大槩思路如下：
